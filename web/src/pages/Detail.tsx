    ...
    <div className="mt-9">
      ...
      {(n.q || n.a) && (
        <div className={`prose-qa grid gap-6 ${one ? 'grid-cols-1' : 'grid-cols-1'}`}>
          {n.q && <div className="q"><h4 className="mb-3 flex items-center gap-2.5 text-[11.5px] font-bold tracking-[.14em] text-ink-3 before:h-px before:w-[22px] before:bg-ink-3">{T('고민', 'Problem')}</h4><div dangerouslySetInnerHTML={{ __html: n.q }} /></div>}
          {n.a && <div className="a"><h4 className="mb-3 flex items-center gap-2.5 text-[11.5px] font-bold tracking-[.14em] text-accent before:h-px before:w-[22px] before:bg-accent">{n.q ? T('해결', 'Solution') : T('내용', 'Detail')}</h4><div dangerouslySetInnerHTML={{ __html: n.a }} /></div>}
        </div>
      )}
      {n.table && (
      ...
      {n.num && (
        <table className="mt-6 w-full border-collapse text-[13.5px] tabular-nums"><thead><tr>{[T('지표', 'Metric'), T('이전', 'Before'), T('결과', 'After'), T('비고', 'Note')].map((h) => <th key={h} className="border-b border-line-2 py-2 pr-3 text-left text-[12.5px] font-medium text-ink-3">{h}</th>)}</tr></thead><tbody>
      ...
      <div className="mt-9 flex justify-between gap-4 border-t border-line-2 pt-5 text-[14px]">
        {i > 0 ? <button type="button" onClick={() => onGo(i - 1)} className="text-left text-ink-2 hover:text-accent"><small className="block text-[12px] text-ink-3">{T('이전 단계', 'Previous step')}</small>{steps[i - 1].t}</button> : <span />}
        {i < total - 1 && <button type="button" onClick={() => onGo(i + 1)} className="text-right text-ink-2 hover:text-accent"><small className="block text-[12px] text-ink-3">{T('다음 단계', 'Next step')}</small>{steps[i + 1].t}</button>}
      </div>
    </div>
  )
}
...
export default function Detail() {
  ...
  if (!it) return <div className="p-10">{T('없는 항목입니다.', 'No such item.')} <Link to="/" className="text-accent underline">{T('홈으로', 'Home')}</Link></div>
  const ORDER = order()
  ...
  return (
    <div className="mx-auto max-w-[1120px] px-5 pb-32 pt-6 sm:px-8 sm:pt-8">
      <button type="button" onClick={close} aria-label={T('닫기', 'Close')} title={T('닫기 (Esc)', 'Close (Esc)')} className="fixed right-5 top-[68px] z-30 flex h-9 w-9 items-center justify-center rounded-full border border-line bg-paper text-ink-2 shadow-[0_4px_14px_-6px_rgba(15,23,42,.3)] hover:border-ink hover:text-ink">
      ...
      <div className="mb-6 flex items-center justify-between text-[13px] text-ink-3">
        <button type="button" onClick={close} className="hover:text-ink">{T('← 타임라인으로 돌아가기', '← Back to timeline')}</button>
        <span>{kind === 'pub' ? T('논문', 'Paper') : T('프로젝트', 'Project')}{kind === 'pub' && it.st ? ` · ${it.st}` : ''}</span>
      </div>
      ...
      <div className="mt-10"><Figs imgs={it.intro.imgs} /><h3 className="mb-2 text-[12.5px] font-medium text-ink-3">{T('개요', 'Overview')}</h3><div className="intro" dangerouslySetInnerHTML={{ __html: it.intro.html }} /></div>

      <div className="sticky top-14 z-[2] mt-11 border-t border-line bg-paper pb-2 pt-4">
        <div className="mb-2.5 text-[12.5px] text-ink-3"><b className="font-medium text-ink">{T('문제 해결 과정', 'How it was solved')}</b>{T(' — 단계를 누르면 아래에 그 단계의 고민과 해결이 열립니다.', ' — pick a step to read the problem and the solution.')}</div>
      ...
      <div className="mt-14 flex flex-wrap justify-between gap-4 border-t border-line pt-6 text-[14px]">
        {prev ? <button type="button" onClick={() => nav(`/p/${prev[0]}/${prev[1].id}`)} className="text-left text-ink-2 hover:text-accent"><small className="block text-[12px] text-ink-3">{T('이전 · 시간순', 'Previous')}</small>{prev[1].t}</button> : <span />}
        {next && <button type="button" onClick={() => nav(`/p/${next[0]}/${next[1].id}`)} className="text-right text-ink-2 hover:text-accent"><small className="block text-[12px] text-ink-3">{T('다음 · 시간순', 'Next')}</small>{next[1].t}</button>}
      </div>
    </div>
  )
}
